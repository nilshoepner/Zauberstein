...
@Component({
  selector: 'app-brettspiele',
  templateUrl: './brettspiele.component.html',
  styleUrls: ['./brettspiele.component.scss']
})
export class BrettspieleComponent {
  events:any;
  
  constructor(private sanitizer: DomSanitizer,private uploadService:UploadServiceService){}

  async download(){
    this.events= await lastValueFrom(this.uploadService.getNewestPerCategory(CategoryEnum.brettspiele));
